# Page notes for KOReader

I wanted to scribble on my books. Little doodles in the margin, a sticker next to a line that made me laugh, a "no. absolutely not." under a bad decision. Kindles don't let you do that, so I made this.
...
When I put out an update, you only need to swap out `main.lua` inside that folder. Your notes and your own stickers live somewhere else, so they won't get touched.
...
**The easy one: highlight something.**
Hold a word or a sentence like you normally would. In the popup, tap **Note / sticker**. The note lands right under the words you picked.
...
Gear icon → **Taps and gestures** → **Gesture manager** → pick a gesture → **General** → **Page notes: add note or sticker**.
...
**Or go through the menu.**
Top menu → wrench icon → **More tools** → **Page notes**.

---

## The popup

This is where you add things.
...
This is where you fiddle with things. A toolbar shows up at the bottom of the screen. If it's covering the spot you need, press on its edge and drag it anywhere.
...
**Arrow row**
- **← ↑ ↓ →** nudges the selected note 4 pixels at a time. E-ink dragging is a bit jumpy, so I use these for the last little bit.

Once you tap Done, notes are just drawn on the page. Tapping and holding go to the book like normal, so you won't accidentally move a sticker while reading.
...
- 256 to 512 pixels wide is plenty. Bigger just takes longer to draw.
...
**Page notes** menu → **New text notes**. Set the font, size, alignment, and whether the white box starts on. Every new text note uses these. You can still change each note on its own afterwards.
...
Nothing personal is stored in the plugin folder. Enjoy, and I'd love to see what you put in your books.
